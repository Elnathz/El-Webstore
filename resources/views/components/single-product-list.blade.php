<div class="flex items-center gap-2 border-b border-gray-200 dark:border-neutral-700">
    <div class="relative overflow-hidden rounded-md h-15 w-15">
        <img class="object-coversize-full"
            src="https://images.unsplash.com/photo-1546087513-2a2bc7fb6fa9?q=80&w=2487&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
            alt="Product Name">
    </div>
    <div class="flex items-center">
        <div class="py-2">
            <h3 class="text-gray-800 text-md dark:text-white">
                Product Name
            </h3>
            <h2 class="text-sm text-gray-500 dark:text-neutral-400">Ebook, Software Engineer, Web Developer</h2>
            <p class="mt-1 text-sm text-black text-md dark:text-white">
                Rp.123.456 x 3
            </p>
        </div>
    </div>
</div>
