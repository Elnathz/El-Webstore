    ...
    <div class="grid grid-cols-1 gap-10 md:grid-cols-10">
                ...
                <div class="block space-y-4">
                    @foreach ($collections as $i => $item)
                        <div class="flex items-center justify-between">
                            <div class="flex">
                                <input type="checkbox" wire:model="selectCollections" value="{{ $item->id }}"
                                ...
                                <label for="hs-default-checkbox-{{ $item->id }}"
                                    class="text-sm font-light ms-3 dark:text-neutral-400">
                                    {{ $item->name }}
                                </label>
                            </div>
                            <span
                                class="text-xs font-light text-gray-800 dark:text-neutral-400">({{ $item->productCount }})</span>
                        </div>
                    @endforeach
                </div>
        ...
        <div class="col-span-1 md:col-span-7">
            <div class="flex items-center justify-between gap-5">
                <div class="font-light text-gray-800 dark:text-neutral-200">Results:
                ...
                <div class="flex items-center gap-2">
                    ...
                    <select wire:model="sortBy"
                        ...
                        <option value="priceAsc">Product Price A-Z</option>
                        <option value="priceDesc">Product Price Z-A</option>
                    </select>
                </div>
            </div>
            <div class="grid grid-cols-1 gap-5 my-5 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
                @forelse ($products as $product)
                    <x-single-product-card :product="$product" wire:key="product-{{ $product->sku }}" />
                @empty
                    <div class="col-span-full text-center text-gray-800 dark:text-neutral-200">No products found</div>
                @endforelse
            </div>
            @if ($products)
                <div>
